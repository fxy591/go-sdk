...
当使用下面列出的智能合约打包器时，将不得不手动执行从Solidity到本机Java类型的所有转换。使用[Solidity smart contract wrappers](https://docs.gbif.io/smart_contracts.html#smart-contract-wrappers)是非常有效的，它负责所有的代码生成和转换。
...
java-sdk负责函数编码，有关实现的进一步细节，请参阅应用程序二进制接口部分[Application Binary Interface](https://docs.gbif.io/abi.html)。
